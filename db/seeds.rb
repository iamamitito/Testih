# This file should contain all the record creation needed to seed the database with its default values.
# The data can then be loaded with the rails db:seed command (or created alongside the database with db:setup).
#
# Examples:
#
#   movies = Movie.create([{ name: 'Star Wars' }, { name: 'Lord of the Rings' }])
#   Character.create(name: 'Luke', movie: movies.first)
User.destroy_all
Centre.destroy_all
AdminUser.destroy_all

User.create(
    name: 'John',
    email:'john@gmail.com',
    password: '123456')

User.create(
    name: 'Jane',
    email:'jane@gmail.com',
    password: '123456')

User.create(
    name: 'Robert',
    email:'robert@gmail.com',
    password: '123456')

User.create(
    name: 'Patrick',
    email:'patrick@gmail.com',
    password: '123456')

User.create(
    name: 'Anita',
    email:'anita@gmail.com',
    password: '123456')

Test.create!(
    date:'2018-11-15',
    user: User.order('RANDOM()').first
    )

Test.create!(
    date:'2019-02-20',
    user: User.order('RANDOM()').first
    )

Test.create!(
    date:'2018-08-23',
    user: User.order('RANDOM()').first
    )

Test.create!(
    date:'2019-04-23',
    user: User.order('RANDOM()').first
    )

Test.create!(
    date:'2017-01-23',
    user: User.order('RANDOM()').first
    )

Test.create!(
    date:'2019-04-10',
    user: User.order('RANDOM()').first
    )

Centre.create(
     name: 'Hospital de Til Til',
     address: 'Daniel Moya 100, Tiltil, Región Metropolitana')

Centre.create(
     name: 'Centro de Salud Familiar Agustín Cruz Melo',
     address: 'Independencia 1325, Independencia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Recoleta',
    address: 'Recoleta 740, Recoleta, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Lucas Sierra',
    address: 'Pedro Fontova 4107, Conchalí, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Quinta Bella',
    address: 'Justicia Social 185, Recoleta, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Valdivieso',
    address: 'Los Cipreses 1892, Recoleta, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar La Pincoya',
    address: 'Recoleta 5580, Huechuraba, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Juan Petrinovic (Ex Scroggie)',
    address: 'Teniente Colipí 670, Recoleta, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Alberto Bachelet Martínez',
    address: 'El Guanaco 3250, Conchalí, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar José Symon Ojeda',
    address: 'El Cortijo 2895, Conchalí, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Colina',
    address: 'Alpatacal 540, Colina, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Irene Frei de Cid',
    address: 'San Luis 525, Quilicura, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Juanita Aguirre',
    address: 'Altona 1835, Conchalí, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Manuel Bustos Huerta',
    address: 'Lo Cruzat 486, Quilicura, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar El Barrero',
    address: 'Las Petunias 305, Huechuraba, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Huertos Familiares',
    address: '4 Oriente 221, Tiltil, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Juan Antonio Ríos',
    address: 'Soberanía 1180, Independencia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Salvador Allende Gossens',
    address: 'Recoleta 6061, Huechuraba, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Presidente Salvador Allende Gossens',
    address: 'Santa Luisa 290, Quilicura, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Cristo Vive (ONG)',
    address: 'Recoleta 4125, Recoleta, Región Metropolitana')

Centre.create(
    name: 'Posta de Salud Rural Las Canteras',
    address: 'Fermín Vergara 159, Colina, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Dr. José Symón Ojeda',
    address: 'Barón de Juras Reales 3988, Conchalí, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar La Foresta',
    address: 'Sotaqui 434, Quilicura, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Pucara de Lasana',
    address: 'Las Torres 211, Quilicura, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Esmeralda',
    address: 'Atena 2008, Colina, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Los Libertadores',
    address: 'San Pedro de Atacama 1925, Huechuraba, Región Metropolitana')

Centre.create(
    name: 'SAR Recoleta',
    address: 'Zapadores 1099, Recoleta, Región Metropolitana')

Centre.create(
    name: 'SAPU Lucas Sierra',
    address: 'Pedro Fontova 4107, Conchalí, Región Metropolitana')

Centre.create(
    name: 'SAPU Valdivieso',
    address: 'Adriana Miranda 1892, Recoleta, Región Metropolitana')

Centre.create(
    name: 'SAR La Pincoya',
    address: 'Recoleta 5580, Huechuraba, Región Metropolitana')

Centre.create(
    name: 'SAPU Alberto Bachelet Martínez',
    address: 'Guanaco 3250, Conchalí, Región Metropolitana')

Centre.create(
    name: 'SAPU Alberto Bachelet Martínez',
    address: 'Guanaco 3250, Conchalí, Región Metropolitana')

Centre.create(
    name: 'SAR Conchalí',
    address: 'Independencia 5663, Conchalí, Región Metropolitana')

Centre.create(
    name: 'SAPU Colina',
    address: 'Carretera General San Martín 77, Colina, Región Metropolitana')

Centre.create(
    name: 'SAPU Nº 2 Irene Frei de Cid',
    address: 'San Luís 525, Quilicura, Región Metropolitana')

Centre.create(
    name: 'SAPU Juan Antonio Ríos',
    address: 'Soberanía 1180, Independencia, Región Metropolitana')

Centre.create(
    name: 'SAPU Presidente Salvador Allende Gossens',
    address: 'Santa Luisa 290, Quilicura, Región Metropolitana')

Centre.create(
    name: 'SAPU Cristo Vive',
    address: 'Recoleta 4125, Recoleta, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Peñaflor',
    address: 'Costa Rica 1000, Peñaflor, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Andes',
    address: 'Andes 4242, Quinta Normal, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Pablo Neruda',
    address: 'Dorsal 6741, Lo Prado, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Lo Franco',
    address: 'Carrascal 4459, Quinta Normal, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Garín',
    address: 'Janequeo 5612, Quinta Normal, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Adalberto Steeger',
    address: 'Huelén 1629, Cerro Navia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Lo Amor',
    address: 'Neptuno 2198, Cerro Navia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Cerro Navia',
    address: 'Sado 7965, Cerro Navia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Raúl Yazigi',
    address: 'Caleta Iquique 214, Lo Prado, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Carlos Avendaño',
    address: 'Teniente Cruz 1087, Lo Prado, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Pudahuel Estrella',
    address: 'La Estrella 1115, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Pudahuel Poniente',
    address: 'San Pablo 9152, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Gustavo Molina',
    address: 'Laguna Sur 8759, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Cardenal Raúl Silva Henríquez',
    address: 'El Tranque 1382, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Arturo Albertz',
    address: 'Las Torres 1204, Cerro Navia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Renca',
    address: 'Balmaceda 4420, Renca, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Bicentenario',
    address: 'Brasil 8005, Renca, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Hernán Urzúa Merino',
    address: 'Totoral Bajo 1011, Renca, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Hernán Urzúa Merino',
    address: 'Totoral Bajo 1011, Renca, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Huamachuco',
    address: 'Las Margaritas 2466, Renca, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Juan Pablo II de Padre Hurtado',
    address: '2ª Transversal 1657, Padre Hurtado, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Fernando Monckeberg',
    address: 'La Concepción 73, Peñaflor, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar El Monte',
    address: 'Los Libertadores 531, El Monte, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Isla de Maipo',
    address: 'Manuel Rodríguez 1001, Isla de Maipo, Región Metropolitana')

Centre.create(
    name: 'Consultorio Coaniquem',
    address: 'San Francisco 8586, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Catamarca',
    address: 'Catamarca 2232, Quinta Normal, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Antumalal',
    address: 'Heriberto Rojas 5967, Quinta Normal, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Antumalal',
    address: 'Lanalhue 1780, Cerro Navia, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Concejal Guillermo Flores O.',
    address: 'Copiapó 1252, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Padre Félix Gutiérrez Donoso',
    address: 'Lago Conguillío 267, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Río Claro',
    address: 'Río Claro 8314, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Mar Caribe',
    address: 'Mar Caribe 1241, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Santa Corina',
    address: 'Relacionador Aranguíz 645, Pudahuel, Región Metropolitana')

Centre.create(
    name: 'SAR Renca',
    address: 'José Manuel Balmaceda 4012, Renca, Región Metropolitana')

Centre.create(
    name: 'SAPU Santa Rosa de Chena',
    address: 'Teniente Sáenz Neira 823, Padre Hurtado, Región Metropolitana')

Centre.create(
    name: 'SAPU Marcela Jacques Vargas',
    address: 'Francisco Chacón 750, Talagante, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar N° 1 Dr. Ramón Corbalán Melgarejo',
    address: 'Copiapó 1323, Santiago, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Nº 5',
    address: 'Unión Latinoamericana 98, Santiago, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Lo Valledor Norte',
    address: 'Central 4000, Pedro Aguirre Cerda, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Maipú',
    address: 'Ramón Freire 2470, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Norman Voulliéme',
    address: 'Salomón Sack 6376, Cerrillos, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar San José de Chuchunco',
    address: 'Coihaique 6025, Estación Central, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Enfermera Sofía Pincheira',
    address: 'Las Torres 7590, Cerrillos, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. José Eduardo Ahués Salame',
    address: 'Independencia 2200, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dra. Ana María Juricic',
    address: 'El Conquistador 1841, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Las Mercedes',
    address: 'Embajador Quintana 5020, Estación Central, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Padre Orellana',
    address: 'Padre Orellana 1706, Santiago, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Arauco',
    address: 'Sepúlveda Leyton 3288, Santiago, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Benjamín Viel',
    address: 'San Ignacio 1217, Santiago, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Iván Insunza',
    address: 'Mar de Chile 491, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Clotario Blest',
    address: 'Los Trabajadores 999, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Clotario Blest',
    address: 'El Conquistador 1565, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Ignacio Domeyko',
    address: 'Cueto 543, Santiago, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Presidenta Michelle Bachelet',
    address: 'Nueva San Martín 776, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Luis Ferrada',
    address: 'Tierra Fértil 5161, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Futramapu',
    address: 'Judea 321, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Europa',
    address: 'Europa 220, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Villa Francia',
    address: 'Las Estepas 1217, Estación Central, Región Metropolitana')

Centre.create(
    name: 'Centro Comunitario de Salud Familiar Bueras',
    address: 'Bueras 456, Maipu, Región Metropolitana')

Centre.create(
    name: 'SAPU Consultorio Nº1',
    address: 'Copiapó 1323, Santiago, Región Metropolitana')

Centre.create(
    name: 'SAPU Maipú',
    address: 'Pajaritos 2470, Maipu, Región Metropolitana')

Centre.create(
    name: 'SAPU San José de Chuchunco',
    address: 'Coihaique 6025, Estación Central, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dra. Ana María Juricic',
    address: 'El Conquistador 1841, Maipu, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Dr. Hernán Alessandri',
    address: 'Los Jesuitas 857, Providencia, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Alfonso Leng',
    address: 'Manuel Montt 303, Providencia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Aguilucho',
    address: 'El Aguilucho 3292, Providencia, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Apoquindo',
    address: 'Cerro Altar 6611, Las Condes, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Aníbal Ariztía',
    address: 'Paúl Harris 1140, Las Condes, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Lo Barnechea',
    address: 'El Rodeo 13533, Lo Barnechea, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Vitacura',
    address: 'Indiana 1195, Vitacura, Región Metropolitana')

Centre.create(
    name: 'Centro de Salud Familiar Rosita Renard',
    address: 'Las Encinas 2801, Ñuñoa, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Salvador Bustos',
    address: 'Grecia 4369, Ñuñoa, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Félix de Amesti',
    address: 'El Líbano 5557, Macul, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santa Julia',
    address: 'Santa Julia 2309, Macul, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar La Faena',
    address: 'Oriental 7250, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar San Luis',
    address: 'La Pradera 5370, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Carol Urzúa Ibáñez',
    address: 'Consistorial 1960, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Ossandon',
    address: 'Echenique 8419, La Reina, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Lo Hermida',
    address: 'Coronel Alejandro Sepúlveda 6931, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Padre Alberto Hurtado',
    address: 'Arturo Prat 4345, Macul, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Juan Pablo II',
    address: 'Parinacota 440, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Padre Gerardo Whelan',
    address: 'Litoral 5960, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro Odontológico Macul',
    address: 'Arturo Prat 4345, Macul, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Lo Barnechea',
    address: 'Circunvalación Norte 5040, Lo Barnechea, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar General Carol Urzúa Ibáñez',
    address: 'Nueva Vida 2087, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Bicentenario',
    address: 'Getsemani 229, Lo Barnechea, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Aníbal Ariztía',
    address: 'La Escuela 1229, Las Condes, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Lo Barnechea',
    address: 'El Rodeo 13533, Lo Barnechea, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Rosita Renard',
    address: 'Las Encinas 2801, Ñuñoa, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santa Julia',
    address: 'Santa Julia 2309, Macul, Región Metropolitana')
    
Centre.create(
    name: 'SAPU La Faena',
    address: 'Los Orientales 7250, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'SAPU San Luis',
    address: 'Los Cerezos 5556, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Lo Hermida',
    address: 'Litoral 5960, Peñalolén, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Lo Hermida',
    address: 'Litoral 5960, Peñalolén, Región Metropolitana')
    
    Centre.create(
    name: 'SAPU Centro de Urgencia Ñuñoa',
    address: 'Juan Moya 1320, Ñuñoa, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Barros Luco',
    address: 'Ángel Guarello 1319, San Miguel, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar San Joaquín',
    address: 'Francisca de Rimini 3223, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Recreo',
    address: 'Varas Mena 785, San Miguel, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Amador Neghme',
    address: 'Alhué 2085, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Villa Sur',
    address: 'Clotario Blest 6035, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Arturo Baeza Goñi',
    address: 'Álvarez de Toledo 399, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Padre Pierre Dubois (Ex La Feria)',
    address: 'Los Comandos 4772, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Clara Estrella',
    address: 'Putre 6868, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Julio Acuña Pinzón',
    address: 'Central Raúl Silva Henríquez 8260, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dra. Mariela Salgado Zepeda',
    address: 'José María Caro 3849, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santa Anselma',
    address: 'Fernando de Aragón 8365, La Cisterna, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Carlos Lorca Tobar',
    address: 'Claudina Parra 11028, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Raúl Cuevas (Ex-San Bernardo)',
    address: 'San Alfonso 1202, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Confraternidad',
    address: 'Martín de Solís 14210, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Carol Urzúa de San Bernardo',
    address: 'Loncomilla 958, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santa Laura',
    address: 'Indio Geronimo 10460, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santa Teresa de Los Andes',
    address: 'Gustavo Campaña 5380, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Mario Salcedo',
    address: 'Mar Báltico 13256, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dra. Haydeé López Casoou',
    address: 'Lo Martínez 862, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Eduardo Frei Montalva',
    address: 'Ossa 140, La Cisterna, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Joan Alsina',
    address: 'Marquez de la Plata 5, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Canciller Orlando Letelier',
    address: 'Lo Moreno 890, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Raúl Brañes F.',
    address: 'Carlos Condell 1036, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Edgardo Enríquez Froeden',
    address: 'Clotario Blest 2650, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Juan Pablo II',
    address: 'Santa Marta 276, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Héctor García',
    address: 'Errazuriz 703, Buin, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar El Manzano',
    address: 'Camino Santa Teresa 1876, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Pueblo Lo Espejo',
    address: 'Lo Espejo 2891, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Yalta',
    address: 'Yalta 2500, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familia Sierra Bella',
    address: 'Sierra Bella 3208, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Cooperación',
    address: 'Cooperacion 6041, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Rapa Nui',
    address: 'Miguel de Unamuno 2738, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Santa Laura',
    address: 'Picton 1050, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Juan Aravena',
    address: 'Juan Aravena 472, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Lo Herrera',
    address: 'Volcan Guallatiri 8282, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Recreo',
    address: 'Varas Mena 785, San Miguel, Región Metropolitana')
    
Centre.create(
    name: 'SAPU La Feria',
    address: 'Uno Oriente 3939, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Padre Esteban Gumucio',
    address: 'Padre Esteban Gumucio 281, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Julio Acuña Pinzón',
    address: 'Central 8260, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dental Valledor Tres',
    address: 'Jose María Caro 3849, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Carlos Lorca',
    address: 'El Pajar 10675, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Raúl Cuevas (Ex San Bernardo)',
    address: 'San Alfonso 1201, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Cóndores de Chile',
    address: 'Gran Avenida 9902, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Confraternidad',
    address: 'Martin de Solis 14210, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dra. Mariela Salgado',
    address: 'Cardenal Caro 3849, Lo Espejo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santa Teresa de Los Andes',
    address: 'Gustavo Campaña 5380, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'SAR Haydee Lopez',
    address: 'Lo Martinez 862, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Eduardo Frei Montalva',
    address: 'Ossa 140, La Cisterna, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Carol Urzúa de San Bernardo',
    address: 'Mendoza 855, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Raúl Brañes F.',
    address: 'Carlos Condell 1036, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Edgardo Enríquez Froedden',
    address: 'Clotario Blest 2650, Pedro Aguirre Cerda, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Juan Pablo II',
    address: 'Santa Marta 277, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Alejandro del Río',
    address: 'Gandarillas 105, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Los Castaños',
    address: 'Diagonal Los Castaños 5820, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Alejandro del Río',
    address: 'Gandarillas 105, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Bellavista',
    address: 'Froilan Roa 6420, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Villa O Higgins',
    address: 'Santa Julia 870, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Los Quillayes',
    address: 'Julio Cesar 10905, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar La Granja',
    address: 'San Gregorio 28, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar La Bandera',
    address: 'Vicuña Mackenna 1758, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar San Rafael',
    address: 'Porto Alegre 12577, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Pablo de Rokha',
    address: 'Pedro Prado 12221, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. José Manuel Balmaceda',
    address: 'Ramon Subercaseaux 87, Pirque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santiago de Nueva Extremadura',
    address: 'Juanita 13558, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar San Gerónimo',
    address: 'San Pedro 1203, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Vista Hermosa',
    address: 'Los Pinos 275, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Salvador Allende (Ex Consultorio San Ramón)',
    address: 'Los Claveles 8097, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Malaquías Concha',
    address: 'Linares 585, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Dr. Fernando Maffioletti',
    address: 'Central 301, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santo Tomás',
    address: 'Santo Tomas 987, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Flor Fernández',
    address: 'Ciudad de Mexico 1503, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar El Roble',
    address: 'Observatorio 1777, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Bernardo Leighton',
    address: 'Miguel Angel 1929, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Cardenal Silva Henríquez',
    address: 'San Pedro 3345, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Padre Manuel Villaseca',
    address: 'Luis Matte Larrain 2312, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Madre Teresa de Calcuta (Organizaciones sin fines de lucro y ONG)',
    address: 'Circunvalacion 1815, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Santa Amalia',
    address: 'Santa Amalia 202, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Granja Sur',
    address: 'Sofía Eastman 10009, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Karol Wojtyla',
    address: 'Curaco de Vélez 4110, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Poetisa Gabriela Mistral',
    address: 'Aurora de Chile 9872, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar San Alberto Hurtado',
    address: 'San Carlos 1693, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Trinidad',
    address: 'Uno 10021, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar La Florida',
    address: 'La Florida 6015, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar Laurita Vicuña',
    address: 'Ejército Libertador 2433, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Rural El Principal',
    address: 'La Alameda 113, Pirque, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Salud Familiar José Alvo',
    address: 'Bacteriológico 10817, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Posta de Salud Rural Las Vertientes',
    address: 'Parque Municipal 2786, San José de Maipo, Región Metropolitana')
    
Centre.create(
    name: 'Posta de Salud Rural Las Perdices',
    address: 'El Panul 6706, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Posta de Salud Rural San Gabriel',
    address: 'Los Rodados 41687, San José de Maipo, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Modelo',
    address: 'Alvear 6543, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar San Gregorio',
    address: 'Angol 8363, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Alejandro del Río',
    address: 'Gandarillas 105, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Alejandro del Río',
    address: 'Gandarillas 105, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Los Castaños',
    address: 'Diagonal Los Castaños 5820, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Villa OHiggins',
    address: 'Santa Julia 870, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Los Quillayes',
    address: 'Julio César 10905, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU La Granja',
    address: 'San Gregorio 28, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'SAPU La Bandera',
    address: 'Vicuña Mackenna 1758, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'SAPU San Rafael',
    address: 'Porto Alegre 12577, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Pablo de Rokha',
    address: 'Pedro Prado 12221, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santiago de Nueva Extremadura',
    address: 'Juanita 13558, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'SAPU San Gerónimo',
    address: 'San Pedro 1203, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU San Ramón',
    address: 'Uruguay 1760, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Fernando Maffioletti',
    address: 'Central 301, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santo Tomás',
    address: 'Santo Tomás 987, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'SAPU El Roble',
    address: 'Observatorio 1777, La Pintana, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Bernardo Leighton',
    address: 'Miguel Ángel 1929, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Cardenal Silva Henríquez de Puente Alto',
    address: 'Estación El Canelo 3345, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAR Padre Manuel Villaseca',
    address: 'Luis Matte Larraín 2312, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santa Amalia',
    address: 'Santa Amalia 202, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Granja Sur',
    address: 'Sofía Eastman 10036, La Granja, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Karol Wojtyla',
    address: 'Curaco de Vélez 4110, Puente Alto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Poetisa Gabriela Mistral',
    address: 'Aurora de Chile 9872, San Ramón, Región Metropolitana')
    
Centre.create(
    name: 'SAPU La Florida',
    address: 'La Florida 6015, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santa Laura',
    address: 'Picton 1050, El Bosque, Región Metropolitana')
    
Centre.create(
    name: 'SAPU San Joaquín',
    address: 'Francisca de Rimini 3223, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Elena Caffarena',
    address: 'Marín 520, Providencia, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Libertadores',
    address: 'San Pedro de Atacama 1925, Huechuraba, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Trinidad',
    address: 'Uno 10021, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Luis Chavarría',
    address: 'Sado 7965, Cerro Navia, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Alberto Allende Jones',
    address: 'Esmeralda 2049, Talagante, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Arturo Albertz',
    address: 'Las Torres 1204, Cerro Navia, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Avendaño',
    address: 'Teniente Cruz 1087, Lo Prado, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Fernando Monckeberg',
    address: 'Concepcion 73, Peñaflor, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Francisco Boris Soler',
    address: 'Silva Chávez 1650, Melipilla, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Gustavo Molina',
    address: 'Laguna Sur 8759, Pudahuel, Región Metropolitana')
    
Centre.create(
    name: 'SAR Dr. Raúl Yazigi',
    address: 'Caleta Iquique 214, Lo Prado, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Garín',
    address: 'Janequeo 5612, Quinta Normal, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Isla de Maipo',
    address: 'Manuel Rodríguez 1001, Isla de Maipo, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Lo Franco',
    address: 'Carrascal 4459, Quinta Normal, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Pablo Neruda',
    address: 'Dorsal 6741, Lo Prado, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Peñaflor',
    address: 'Costa Rica 1000, Peñaflor, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Violeta Parra',
    address: 'La Travesía 7447, Pudahuel, Región Metropolitana')
    
Centre.create(
    name: 'SAPU María Pinto',
    address: 'Francisco Costabal 78, Maria Pinto, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Bicentenario',
    address: 'Brasil 8005, Renca, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Adalberto Steeger',
    address: 'Huelén 1629, Cerro Navia, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Edelberto Elgueta',
    address: 'Arza 1576, Melipilla, Región Metropolitana')
    
Centre.create(
    name: 'SAPU El Monte',
    address: 'Los Libertadores 531, El Monte, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Huamachuco',
    address: 'Las Margaritas 2466, Renca, Región Metropolitana')
    
Centre.create(
    name: 'SAR La Estrella',
    address: 'El Lazo 8631, Pudahuel, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Pudahuel Poniente',
    address: 'San Pablo 9152, Pudahuel, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Santa Anita',
    address: 'Camino de Loyola 5302, Lo Prado, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Dr. Iván Insunza',
    address: 'Mar de Chile 491, Maipu, Región Metropolitana')
    
Centre.create(
    name: 'SAPU Aguilucho',
    address: 'El Aguilucho 3235, Providencia, Región Metropolitana')
    
Centre.create(
    name: 'SAPU José Alvo',
    address: 'Bacteriológico 10817, La Florida, Región Metropolitana')
    
Centre.create(
    name: 'Centro de Especialidades Odontológicas Leng',
    address: 'Eliodoro Yáñez 1261, Providencia, Región Metropolitana')
    
Centre.create(
    name: 'SAR Enfermera Sofía Pincheira',
    address: 'Las Torres 7590, Cerrillos, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Coñimo',
    address: 'Catruman 275, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Villa Olímpica',
    address: 'Salvador 1350, Ñuñoa, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Andacollo',
    address: 'Andacollo 1661, Providencia, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Beato Padre Hurtado',
    address: 'Parroquia Nuestra Señora del Carmen 551, Quilicura, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familia Martin Henríquez',
    address: 'Martin Henríquez 4537, San Joaquín, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Salud Plaza México',
    address: 'Huérfanos 4490, Quinta Normal, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Buzeta',
    address: 'Carlos Valdovinos 3955, Cerrillos, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Alberto Bachelet',
    address: 'Cunaco 1424, Conchalí, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Eduardo Frei Montalva',
    address: 'Victoria 8396, La Cisterna, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Atacama',
    address: 'Segunda Avenida 1462, San Miguel, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Dr. Raúl Brañes',
    address: 'Carlos Condell 1789, San Bernardo, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar San Vicente de Naltagua',
    address: 'San Antonio de Naltagua 7603, Isla de Maipo, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Amapolas',
    address: 'Amapolas 4899, Ñuñoa, Región Metropolitana')
    
Centre.create(
    name: 'Centro Comunitario de Salud Familiar Las Lomas',
    address: 'Jardin Alto 9057, La Florida, Región Metropolitana')



    #PrEP
Centre.create(
    name: 'Hospital Barros Lucos',
    address: 'Jose Miguel Carrera 3204, San Miguel, Región Metropolitana',
    kind: 'prep')
    
Centre.create(
    name: 'Hospital San Juan de Dios',
    address: 'Huérfanos 3255, Santiago, Región Metropolitana',
    kind: 'prep')
    
Centre.create(
    name: 'Hospital El Carmen',
    address: 'Camino A Rinconada 1201, Maipú, Región Metropolitana',
    kind: 'prep')
    
Centre.create(
    name: 'CRS Cordillera Norte',
    address: 'Las Torres 5100, Peñalolén, Región Metropolitana',
    kind: 'prep')
    
Centre.create(
    name: 'Hospital Padre Hurtado',
    address: 'Esperanza 2150, San Ramón, Región Metropolitana',
    kind: 'prep')
    
Centre.create(
    name: 'Hospital San José',
    address: 'San José 1196, Independencia, Región Metropolitana',
    kind: 'prep')

    
    


    
AdminUser.create!(email: 'admin@example.com', password: 'password', password_confirmation: 'password') 